// This code is created by Ahmed A. Eltahan

/* 
	----------------------------- Program Description -----------------------------------------
	Write C code that contains two functions; PrintMyName and main. PrintMyName is 
	defined in file2.c and its role is to print your name. main is define in file1.c and its role 
	is to call PrintMyName function
	
	file2 : contains only the needed libraries and the function declararion and body without main
*/

// Libraries
#include<stdio.h> // Library to include c functions for the compiler, just to get rid of the warning.

// Declared Functions
void PrintMyName(void)
{
	printf("My name is Ahmed A. Eltahan");
}

