...
/* 
	...
	runtime number using calloc function.
*/
...
{	
	...
	ptr = (int*) calloc(noe, sizeof(int)); // takes 2 input variables
	
		// Outputing virtual array elements using ptr[i] Before filling (Inintialization of Zero Values)
			printf("Array elements using ptr[i] before filling are zero values as: \n");
			for (i = 0; i < noe; i++)
			{
				printf("%d \t",ptr[i]);
			}
