 ...
 */
	...
	the normal mode of wave generation method. It's required to blink 2 LEDs connected on C0, C1 with the concept of timers with the following signals.
...
// Pins Selected
#define LED1_Pin PIN16
#define LED2_Pin PIN17
...
// Global variables (Declaration and Initialization)
	...
	u8 LED1_State = LED_OFF; // Initially, LED1 is off
	u8 LED2_State = LED_OFF; // Initially, LED2 is off
	u8 counter = 0; // counter used to regulate the toggle of the state of the above two signals
...
void main(void)
{
	...
	SetPinDirection(LED1_Pin, OUTPUT); // define LED1 pin as output
	SetPinDirection(LED2_Pin, OUTPUT); // define LED2 pin as output
...
ISR(__vector_11) // Timer/Counter0 Overflow vector no. (12) - 1
{
	...
	counter++; // to regulate the toggle of the state of the above two signals

	// --------------------------------------- One Seconds Starts ---------------------------------------------
	if (overflows == OneSecCounts_1024)
	{
		if (counter%2 == 1) // odd number state
		{
			SetPinValue(LED1_Pin, TOG_BIT(LED1_State, 0));
			SetPinValue(LED2_Pin, TOG_BIT(LED2_State, 0));

			overflows = 0; // reset the overflows counter
		}
		else // counter%2 == 0 // even number state
		{
			SetPinValue(LED1_Pin, TOG_BIT(LED1_State, 0));
