...
// Time Required Number of overflows and initial value of TCNT0 Macros (prescaler 8)
	// One milliSecond
	#define OnemilliSecCounts_8     4 // to check the number of loops
	#define OnemillSecInitTCNT0_8   23 // to match the partial number of the part of a count (non full overflow)
...
void main(void)
{
	...
	// Module Initialization
		// Timers Initialization
			...
			TCNT0 = OnemillSecInitTCNT0_8; // initially for the fractional part for 1 second
			CLR_BIT(TCCR0, CS02); SET_BIT(TCCR0, CS01); CLR_BIT(TCCR0, CS00); // Start the timer TCCR0 = 0b00000101; prescaler 8
...
ISR(__vector_11) // Timer/Counter0 Overflow vector no. (12) - 1
{
	...
	// --------------------------------------- One milliSeconds Starts ---------------------------------------------
	if ( overflows == OnemilliSecCounts_8 )
	{
		SetPinValue(LED_Pin, TOG_BIT(LED_State, 0));
		overflows = 0; // reset the counter
		TCNT0 = OnemillSecInitTCNT0_8; // For the next loop
	}
	// --------------------------------------- One milliSeconds Ends with (LED OFF) --------------------------------
