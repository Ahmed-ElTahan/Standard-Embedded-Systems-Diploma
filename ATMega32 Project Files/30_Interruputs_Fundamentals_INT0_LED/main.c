...
void main(void)
{
	...
	SetPinDirection(LED_Pin, OUTPUT);  // LED pin output declaration
...
{
	TOG_BIT(LED_State, 0); // change only bit no 0 for the LED state
	SetPinValue(LED_Pin, LED_State); // write the LED state value toggled to the LED
	_delay_ms(250); // for de-bouncing
}
