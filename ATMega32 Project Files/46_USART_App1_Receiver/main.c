...
void main(void)
{
		...
		// Module Initialization
			// UART Initialization
				...
				// UCSRB
					...
					SET_BIT(UCSRB, TXEN); // enable tranmission mode
				...
				SetPinDirection(PIN1, OUTPUT); // LED2 A1
				SetPinDirection(PIN2, OUTPUT); // LED3 A2
	...
	while(1)
	{
		...
		data = USART_u8ReceiveData(); // receive data
		SetPinValue(PIN0, HIGH); // LED1 on

		if(data == 'A')
		{
			USART_vidSendData('B'); // send char B
			SetPinValue(PIN1, HIGH); // LED2 on A1
		}
		else
		{
			SetPinValue(PIN2, HIGH); // LED3 on A2
		}

		// reset the leds for the right next loop after 1st loop if received garbage value
		_delay_ms(1000);
		PORTA = 0;
	}
