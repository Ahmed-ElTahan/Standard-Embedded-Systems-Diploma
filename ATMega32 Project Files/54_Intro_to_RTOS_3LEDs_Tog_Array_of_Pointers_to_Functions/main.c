...
// Declared Functions
	...
	// Array of pointers to functions (declaration and Initialization of previous functions)
	void (*ArrPtr[3]) (void) = {Green, Red, Blue};
...
ISR(__vector_11) // Timer/Counter0 Overflow vector no. (12) - 1
{
	...
	{
		if(counter%3 == 0 ) 	 {ArrPtr[0]();}
		else if (counter%3 == 1) {ArrPtr[1]();}
		else if (counter%3 == 2) {ArrPtr[2]();}
