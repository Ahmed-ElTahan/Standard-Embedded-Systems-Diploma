	...
	while(1)
	{
		// Write your instructions here.
			...
			// --------------------------------------- Two Seconds Starts ---------------------------------------------
				// initially
				overflows = 0;
				TCNT0 = TwoSecInitTCNT0_1024;

				// start count
				SET_BIT(TCCR0, CS02); CLR_BIT(TCCR0, CS01); SET_BIT(TCCR0, CS00); // TCCR0 = 0b00000101; prescaler 1024

				while (overflows <= TwoSecCounts_1024) // stuck here till the count you want
				{
			...
			// the LED exits with ON state and will wait for 1 second in the next part with this state
