...
/*
	...
	(only Read ADCH register) and print this digital value (0-255) on 1 LED connected on C0
	...
	We want to determine conditions of Light condition to turn off the LED, and Dark condition to turn on the LED
...
void main(void)
{
	...
	//SetPinDirection(PIN16, OUTPUT); // define C0 output for the LED
	DDRC = 0xFF;
	...
	while(1)
	{
		...
		if ((data >=2) && (data <= 25)) // Dark Condition
		{
			//SetPinValue(PIN16, HIGH);
			PORTC = 0xFF;
		}

		else // Light Condition
		{
			//SetPinValue(PIN16, LOW);
			PORTC = 0x00;
		}
	}
